...
class TrainLoop:
    def __init__(self, args, train_platform, model, diffusion, data,dataset,dataset_test):
        ...
        #self.data_len = 6000
        #self.num_epochs = self.num_steps // self.data_len + 1
        self.cfg = Config(self.args.cfg, test=self.args.test)
        self.len = self.cfg.num_vae_data_sample//self.cfg.batch_size
        self.num_epochs = self.num_steps // self.len + 1
    ...
    def run_loop(self):
        #print("daxiao",self.num_epochs)
        for epoch in range(self.num_epochs):
            print(f'Starting epoch {epoch}')
            # 原始未加噪声的动作序列，与动作序列相关的条件信息，例如文本嵌入、动作提示等
            #cfg = Config(self.args.cfg, test=self.args.test)
            #t_his = cfg.t_his
            #cfg = Config(self.args.cfg, test=self.args.test)
            data_train = self.dataset_train.sampling_generator(num_samples=self.cfg.num_vae_data_sample, batch_size=self.cfg.batch_size,
                                           is_other_act=self.args.is_other_act, t_pre_extra=self.args.t_pre_extra,
                                           act_trans_k= self.cfg.vae_specs['act_trans_k'] if 'act_trans_k'
                                                                                        in self.cfg.vae_specs else 0.08,
                                           max_trans_fn= self.cfg.vae_specs['max_trans_fn'] if 'max_trans_fn'
                                                                                        in self.cfg.vae_specs else 0.08,
                                           is_transi=self.args.is_transi)
            #print("t",self.args.t_pre_extra)
            for motion,label, fn, fn_mask, cond in tqdm(data_train):
    ...
    def evaluate_kai(self):
        cfg = Config(self.args.cfg, test=self.args.test)
        
        data_test = self.dataset_test.sampling_generator(num_samples=cfg.num_vae_data_sample, batch_size=cfg.batch_size,
                                is_other_act=self.args.is_other_act, t_pre_extra=self.args.t_pre_extra,
        ...
        for motion, label, fn, fn_mask, cond in tqdm(data_test):
            if not (not self.lr_anneal_steps or self.step + self.resume_step < self.lr_anneal_steps):
                break
            motion = motion.to(self.device)
            cond['y'] = {key: val.to(self.device) if torch.is_tensor(val) else val for key, val in cond['y'].items()}
            ...
            for i in range(0, batch.shape[0], self.microbatch):
                ...
                for k, v in losses.items():
                    v = (v * weights).mean().item()
                    total_losses[k] = total_losses.get(k, 0.0) + v
                count += 1

        # 取平均
        avg_losses = {k: v / count for k, v in total_losses.items()}

        # log 到平台
        self.log_step()
        for k, v in avg_losses.items():
            if k == 'loss':
                print(f'test[{self.step + self.resume_step}]: loss[{v:.5f}]')
            self.train_platform.report_scalar(name=k, value=v, iteration=self.step, group_name='Test')
