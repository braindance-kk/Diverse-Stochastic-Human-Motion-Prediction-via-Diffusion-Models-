...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        if self.model_mean_type == ModelMeanType.PREVIOUS_X:
            ...
            #print("x_start shape",pred_xstart.shape)
    ...
    def model_output(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None):
        if model_kwargs is None:
            model_kwargs = {}
        if noise is None:
    ...
    def rot6d_to_axisangle(self,rot6d):
        """
        Args:
        ...
        return axis_angle
    def check_rot6d_validity(self,rot6d):
        # rot6d: [..., 6]
        a1 = rot6d[..., :3]
        a2 = rot6d[..., 3:]
        ...
        # 重建 rotation matrix
        rotmat = torch.stack([b1, b2, torch.cross(b1, b2, dim=-1)], dim=-2)  # [..., 3, 3]
        identity = torch.eye(3).to(rot6d.device)
        ortho_err = torch.matmul(rotmat.transpose(-2, -1), rotmat) - identity
        print("rot6d orth error (mean abs):", ortho_err.abs().mean().item())
    ...
    def training_losses(self, model,x_start, t, model_kwargs=None, noise=None, dataset=None):
        ...
        #序列训练条件 originnal [batch, njoints, nfeats, sequence_length]
        ...
        # add noise to the clean motion sequences
        x_t = self.q_sample(x_start, t, noise=noise)
        ...
        # MSE loss
            ...
            #print("x_rotations.shape:", x_rotations.shape)
            #print("mask.shape:", mask.shape)
            #print("x_rotations[mask].shape:", x_rotations[mask].shape)
            ...
            if self.lambda_vel > 0.:
                ...
                terms["vel_mse"] = self.masked_l2(
                    target_vel[:, 1:, :, :],           # 跳过 root joint
                    model_output_vel[:, 1:, :, :], 
                    mask[:, :, :, 1:]                 # mask 同样需要跳过
                )

            # get two smoothness loss
            
            dct_m,idct_m = get_dct_matrix(20,is_torch = True, device = x_start.device)
            dct_m = dct_m.float()
            idct_m = idct_m.float()

            x_his = model_kwargs['y']['samp_his']
            B,J,F,T_his = x_his.shape
            x_his = x_his.permute(0, 3, 1, 2).contiguous()  # [B, J, 6, T] → [B, T, J, 6]
            #x_his = x_his.permute(3,0,1,2).reshape(T_his,B,J*F).contiguous()

            fn_mask = model_kwargs['y']['fn_mask']
            #print("fn_mask shape:", fn_mask.shape)

            fn_mask = fn_mask[:,T_his:]
            
            Y_r = model_output.permute(0, 3, 1, 2).contiguous()  # [B, J, 6, T] → [B, T, J, 6]
            #B,J,F,T_pred = model_output.shape
            #Y_r = model_output.permute(3,0,1,2).reshape(T_pred,B,J*F).contiguous()
            
            Y = x_start.permute(0, 3, 1, 2).contiguous()  # [B, J, 6, T] → [B, T, J, 6]
            #B,J,F,T_pred = x_start.shape
            #Y = x_start.permute(3,0,1,2).reshape(T_pred,B,J*F).contiguous()

            #self.check_rot6d_validity(Y)
            """
            
            if dataset is not None:
                mean = torch.tensor(dataset.mean, dtype=x_start.dtype, device=x_start.device)
                std = torch.tensor(dataset.std, dtype=x_start.dtype, device=x_start.device)

            Y_r[:, :, 1:] = Y_r[:, :, 1:] * std + mean
            x_his[:, :, 1:] = x_his[:, :, 1:] * std + mean
            """
            Y_r = self.rot6d_to_axisangle(Y_r) # [B, T, J, 6]→[B, T, J, 3]
            #print("Axis-angle max:", Y_r.abs().max().item())
            
            x_his = self.rot6d_to_axisangle(x_his)
            Y = self.rot6d_to_axisangle(Y)

            B,T_his,J,F = x_his.shape
            x_his = x_his.permute(1, 0, 2, 3).contiguous().view(T_his, B, J * F)  # # [B, T, J, 3]→[T, B, J*3]
            
            B,T,J,F = Y_r.shape
            Y_r = Y_r.permute(1, 0, 2, 3).contiguous().view(T, B, J * F)

            B,T,J,F = Y.shape
            Y = Y.permute(1, 0, 2, 3).contiguous().view(T, B, J * F)

            MSE = (Y_r - Y).pow(2).sum(dim=-1).transpose(0, 1)[fn_mask == 1].mean()

            X = torch.cat([x_his[-10:],Y_r[:10]],dim=0).transpose(0,1)
            x_est = torch.matmul(idct_m[None,:,:5],torch.matmul(dct_m[None,:5],X))
            MSE_v1 = (x_est-X).norm(dim = -1).mean()
            MSE_v2 = (x_his[-1] - Y_r[0]).norm(dim = -1).mean()
            
            terms["SMPL_MSE"] = MSE
            terms["dct"]  = MSE_v1
            terms["transition"] = MSE_v2
            
            terms["loss"] = terms["rot_mse"] + terms.get('vb', 0.) +\
                            (self.lambda_vel * terms.get('vel_mse', 0.)) +\
                            (self.lambda_rcxyz * terms.get('rcxyz_mse', 0.)) + \
                            (self.lambda_fc * terms.get('fc', 0.)) + 0.1*terms["transition"] + 0.1*terms["dct"] + terms["SMPL_MSE"]
...
def _extract_into_tensor(arr, timesteps, broadcast_shape):
    ...
    """
    #print("t shape:", timesteps.shape)
    #print("x_start shape:", broadcast_shape)
