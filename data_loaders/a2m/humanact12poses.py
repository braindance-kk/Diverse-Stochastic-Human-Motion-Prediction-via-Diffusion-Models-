...
class HumanAct12Poses(Dataset):
    ...
    def __init__(self, datapath="dataset/HumanAct12Poses", split="train", **kargs):
        ...
        '''
        #检查每个关键字段的大小
        print("Poses shape:", len(data["poses"]))
        print("Joints shape:", len(data["joints3D"]))
        print("Actions shape:", len(data["y"]))

        # 查看单个样本的详细尺寸
        print("Single pose sample shape:", data["poses"][0].shape)
        print("Single joints3D sample shape:", data["joints3D"][0].shape)
        '''
