...
class DatasetBabel(Dataset):
    ...
    def __getitem__(self, idx):
        # 根据 idx 生成对应的样本
        #action = self.act if self.act else np.random.choice(self.act_name)
        seq_his, seq_gt, fn_gt, fn_mask_gt, label_gt = self.sample(action=None,is_other_act=self.is_other_act,
                                                                           t_pre_extra=self.t_pre_extra,
                                                                           k=self.act_trans_k,max_trans_fn=self.max_trans_fn)
        
        # 将数据转换为 torch 张量
        seq_his = torch.tensor(seq_his, dtype=torch.float32)
        seq_gt = torch.tensor(seq_gt, dtype=torch.float32)
        fn_gt = torch.tensor(fn_gt, dtype=torch.float32)
        fn_mask_gt = torch.tensor(fn_mask_gt, dtype=torch.float32)
        label_gt = torch.tensor(label_gt, dtype=torch.float32)
        
        # 将 motion 和 cond 数据打包
        #motion = torch.cat([seq_his, seq_gt], dim=1)  # 按需要的维度合并
    ...
    def sampling_generator(self, num_samples=1000, batch_size=8,act=None,is_other_act=False,t_pre_extra=0,
        ...
        for i in range(num_samples // batch_size):
            ...
            fn_mask = torch.tensor(fn_mask).unsqueeze(1).unsqueeze(1)  # 先在第 1 维和第 2 维添加维度
